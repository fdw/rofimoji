...
class Rofimoji:
    ...
    def resolve_file(self, file_name: str) -> List[Path]:
        ...
        if Path(file_name).expanduser().is_file():
            ...
            nested_file_names = [
                self.resolve_file(file.stem) for file in (Path(__file__).parent / "data").glob("*.csv")
            ]
    ...
    def format_recent_characters(self) -> str:
        pairings = [
            f"\u200e{(index + 1) % 10}: {character}"
            for index, character in enumerate(self.load_recent_characters(self.args.max_recent))
        ]
    ...
    def process_chosen_characters(self, chosen_characters: List[str]) -> str:
        ...
        processed_characters = "".join(
            self.add_skin_tone(self._parse_line(character)) for character in chosen_characters
        )
